...
import { faEdit, faEye, faFilter } from '@fortawesome/free-solid-svg-icons';
import { StorageService } from 'src/app/services/storage.service';
import { MasterService } from '../../master.service';

@Component({
  selector: 'app-broker-memo-list-view',
  templateUrl: './broker-memo-list-view.component.html',
  styleUrls: ['./broker-memo-list-view.component.scss']
})
export class BrokerMemoListViewComponent implements OnInit {
  ...
  // go to the edit  page
  navigateToEdit() {
    this.router.navigateByUrl("home/operation/broker-memo/Upsert/" + 'Edit', {state: this.selectedTrip});
  }
  navigateToDetails() {
    this.router.navigateByUrl("home/operation/broker-memo/Upsert/" +'Preview',{state: this.selectedTrip});
  }
  // get customer
  ...
  search() {
    ...
    if(this.logInUser && this.logInUser.user_type && this.logInUser.user_type.length){
      ...
      this.masterService.getAllBrokerMemo(request)
        .subscribe((data: any) => {
          if (data) {
